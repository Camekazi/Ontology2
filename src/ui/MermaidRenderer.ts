import { Ontology, OntologyNode, OntologyEdge } from '../core/types';
import { DEFAULT_TAXONOMY_CLASSES } from '../taxonomy/DefaultTaxonomy';
...
export class MermaidRenderer {
  ...
  public renderOntology(ontology: Ontology): string {
    const nodes = this.limitNodes(ontology.nodes);
    const edges = ontology.edges.filter(edge => 
  ...
  public renderSimpleGraph(entities: string[], relations: Array<{source: string, target: string, label: string}>): string {
    let mermaid = `flowchart ${this.options.direction}\n`;
  ...
  private limitNodes(nodes: OntologyNode[]): OntologyNode[] {
  ...
  private getNodeImportance(node: OntologyNode): number {
  ...
  private renderNode(node: OntologyNode): string {
  ...
  private renderEdge(edge: OntologyEdge): string {
  ...
  private renderStyling(nodes: OntologyNode[]): string {
    let styling = '\n';
  ...
  private getTypeColor(type: string): string | null {
    const taxonomyClass = DEFAULT_TAXONOMY_CLASSES.find(tc => tc.id === type);
  ...
  public renderLegend(): string {
    ...
    for (const taxonomyClass of DEFAULT_TAXONOMY_CLASSES) {
  ...
  public renderWithLegend(ontology: Ontology): string {
