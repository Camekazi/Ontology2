...
export class RelationExtractor {
  private options: RelationExtractionOptions;
  private defaultPatterns: RelationPattern[];
  ...
  public extractRelations(text: string, entities: Entity[]): RelationClass[] {
    const relations: RelationClass[] = [];
    ...
    for (const sentence of sentences) {
      ...
      for (let i = 0; i < sentenceEntities.length; i++) {
        for (let j = i + 1; j < sentenceEntities.length; j++) {
          ...
          const extractedRelations = this.findRelationsBetweenEntities(
            sentenceText, 
            entity1, 
            entity2, 
            allPatterns,
    ...
    return relations.filter(relation => 
      relation.confidence >= (this.options.minConfidence || 0.3)
    );
  }

  private createEntityPositionMap(entities: Entity[]): Map<string, { start: number; end: number }[]> {
  ...
  private findRelationsBetweenEntities(
    ...
    for (const pos1 of entity1Positions) {
      for (const pos2 of entity2Positions) {
        ...
        if (distance > (this.options.maxDistance || 100)) continue;
  ...
  private createRelation(
    ...
    const relation = new RelationClass(
      sourceId,
      targetId,
      label,
      adjustedConfidence,
  ...
  public addCustomPattern(pattern: RelationPattern): void {
  ...
  public getAvailablePatterns(): RelationPattern[] {
